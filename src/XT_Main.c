...
int main(int argc, char **argv)
{
	...
	float *magobject, *data_unflip_x, *data_unflip_y, *proj_angles_x, *proj_angles_y, vox_wid, qggmrf_sigma, qggmrf_c,  convg_thresh, admm_mu, data_var;
	FILE *debug_msg_ptr;
...
/*Function which parses the command line input to the C code and initializes several variables.*/
{
	...
	static struct option long_options[] =
		...
		Note that attenuation coefficient is what we are trying to reconstruct.*/
               {"regparam_beta",  required_argument, 0, 'e'}, /*Spatial regularization parameter.*/
	...
	*data_var = 1;
	while(1)
	{		
	   c = getopt_long (argc, argv, "a:b:c:d:e:g:h:i:jk:l:m:n:", long_options, &option_index);
	...
	*data_var = 1.0;
	*qggmrf_sigma = 1.0/(sqrt(regparam_beta)+EPSILON_ERROR);
	*qggmrf_c = EPSILON_ERROR;
